...
namespace Announcement.Android.Controls
{
	...
	public class ScaleImageView : ImageView, View.IOnTouchListener
	{
		...
		public CanScroll Cutting()
		{
			...
			if (TranslateY == -(height - m_Height) && TranslateY == 0)
				return CanScroll.TopBottom;
			else if (TranslateY == -(height - m_Height))
				return CanScroll.Bottom;
			else if (TranslateY == 0)
				return CanScroll.Top;
			else
				return CanScroll.CantScroll;
		}
		...
		public override bool OnTouchEvent(MotionEvent e)
		{
			if (m_GestureDetector.OnTouchEvent(e))
			{
				...
				if (!(e.GetX () > 40 
					&& e.GetX () < MainActivityInstance.Current.Resources.DisplayMetrics.WidthPixels - 40)) 
				{
					if(Parent != null)
			...
			case MotionEventActions.Move:
				{
					if (touchCount >= 2 && m_IsScaling)
					{
						...
						if(Parent != null)
							Parent.RequestDisallowInterceptTouchEvent (m_IsScaling);
					}
					else if (!m_IsScaling)
					{
						...
						var canScroll = this.Cutting ();
						bool cs;
						if (canScroll == CanScroll.TopBottom)
								cs = true;
						else if (distanceY > 0 && canScroll == CanScroll.Bottom)
								cs = true;
						else if (distanceY < 0 && canScroll == CanScroll.Top)
								cs = true;
							else
								cs = false;
						
						if(Parent != null)
							Parent.RequestDisallowInterceptTouchEvent (!cs);
					}
				}
				break;

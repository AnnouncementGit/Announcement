...
{
	public Result<List<UserReadOnly>> PullModerators(UserCredentials user, Context context) 
    {
        Result<List<UserReadOnly>> result = new Result<List<UserReadOnly>>();
        ...
        try 
        {
			...
			if (TokenValitation(user, ADMINISTRATOR, mapper)) 
			{
				Condition condition = new Condition();

				condition.withComparisonOperator(ComparisonOperator.EQ.toString());

				condition.withAttributeValueList(new AttributeValue().withN(String.valueOf(MODERATOR)));

				DynamoDBScanExpression expression = new DynamoDBScanExpression();

				expression.addFilterCondition("Role", condition);

				result.setValue(mapper.scan(UserReadOnly.class, expression));
